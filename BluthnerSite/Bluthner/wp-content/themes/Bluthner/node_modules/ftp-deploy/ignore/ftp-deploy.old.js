...
const FtpDeployer = function() {
    ...
    function ftpPut(partialFilePath, cb) {
        let remoteFilePath = path.join(remoteRoot, partialFilePath).replace(/\\/g, '/');
        ...
        ftp.put(fullLocalPath, remoteFilePath, err => {
            if (err) {
                console.log("Put error", fullLocalPath, err);
                emitData.err = err;
            ...
            if (err1) {
                ...
                ftpMakeDirectoriesIfNeeded(err => {
                    if (err) {
                        ...
                        ftpPut(partialFilePaths[0], putErr => {
                        // async.eachSeries(partialFilePaths, ftpPut, putErr => {
                            if (putErr) {
                                console.log("puts failed", putErr);
                            }

                            setTimeout(() => {
                                ftp.raw('quit', (err, data) => {
                                    if (err) {
                                        console.log("Closing connection failed", err);
                                    }
                                    cb(err);
                                });
                            }, 2000);
